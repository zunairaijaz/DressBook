...
const SideFilter: React.FC = () => {
    ...
    return (
        <form className="space-y-8">
            <div>
                <button type="button" onClick={clearFilters} className="text-sm text-gray-500 hover:text-gray-800">Clear all filters</button>
            </div>
             <div>
                ...
                <div className="mt-4 space-y-2">
                    {genderFilters.map(gender => (
                        <div key={gender} className="flex items-center">
                            ...
                            />
                            <label htmlFor={`gender-${gender}`} className="ml-3 text-sm text-gray-600">{gender}</label>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div>
                ...
                <div className="mt-4">
                    <form className="flex items-center space-x-2" onSubmit={handlePriceSubmit}>
                        <input
                            type="number"
                            name="min"
                            value={price.min}
                        ...
                        />
                        <button type="submit" className="px-3 py-2 bg-gray-200 text-sm rounded-md hover:bg-gray-300">Go</button>
                    </form>
                </div>
            </div>
                    ...
                    {allMaterials.map(material => (
                        <div key={material} className="flex items-center">
                            <input
                                ...
                                className="h-4 w-4 rounded border-gray-300 text-accent focus:ring-accent"
                            />
                            <label htmlFor={`material-${material}`} className="ml-3 text-sm text-gray-600">{material}</label>
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    {allPatterns.map(pattern => (
                        <div key={pattern} className="flex items-center">
                            <input
                                ...
                                className="h-4 w-4 rounded border-gray-300 text-accent focus:ring-accent"
                            />
                            <label htmlFor={`pattern-${pattern}`} className="ml-3 text-sm text-gray-600">{pattern}</label>
                        </div>
                    ))}
                </div>
            </div>
                ...
                <h3 className="text-lg font-medium text-gray-900">Availability</h3>
                 <div className="mt-4 flex items-center">
                    <input
                        ...
                        className="h-4 w-4 rounded border-gray-300 text-accent focus:ring-accent"
                    />
                    <label htmlFor="availability" className="ml-3 text-sm text-gray-600">Include Out of Stock</label>
                </div>
            </div>
